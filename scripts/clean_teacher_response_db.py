...
error_videos = []
db = json.load(open("checkpoints/datasets/json/qwen_annotations.json"))
for video in db.keys():
    try:
        parsed_response = TeacherResponse(**db[video])
    except Exception as e:
        print(f"Error parsing {video}")
        error_videos.append(video)

filtered_data = {k: v for k, v in db.items() if k not in error_videos}

print(len(error_videos))
print(len(filtered_data.keys()))
with open("scripts/qwen_annotations_cleaned.json", "w") as f:
